...
def blinker_static(args):
    ...
    res = ana.analyze(
        ...
        variables={"vc": 9},
        start_voltages={"t1.C": 4, "t2.C": 0.1, "t1.B": 0.1},
    )
    res.display()
...
def try_random_start(args):
    ...
    res = ana.analyze(
        ...
        variables={"vc": 9},
        transient=True,
        start_voltages={"t1.C": args.t1c, "t1.B": args.t1b},
    )
    if isinstance(res, str):
        ...
        for s in ["t1.C", "t1.B", "t2.C", "t2.B", "ca.p", "ca.n"]:
            print(
                "{0} V = {1}   I={2}".format(s, res.get_voltage(s), res.get_current(s))
            )


def blinker3(args):
    net = create_blinker(
        transistor_gain=args.gain, cutoff=args.cutoff, rampup=args.rampup
    )
    ana = Analysis(net)
    ...
    res = ana.transient(
        ...
        variables=variables,
        maxit=200,
        start_voltages={"t1.C": args.t1c, "t1.B": args.t1b, "t2.C": args.t2c},
    )
