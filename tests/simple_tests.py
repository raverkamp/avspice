...
class TestSolve(unittest.TestCase):
    ...
    def test_simple_solve1(self):
        ...
        (sol, y, dfx, iterations, norm_y) = solving.solve(
            np.array([1, 2]), f, Df, 1e-8, 1e-8
        )
...
class Test1(unittest.TestCase):
    ...
    def test_diode_simple2(self):
        ...
        self.assertAlmostEqual(
            res.get_voltage("d1.p") - res.get_voltage("d1.n"),
...
class TestTransistor(unittest.TestCase):
    ...
    def test_trans1(self):
        ...
        self.assertAlmostEqual(
            res.get_current("t1.C") / res.get_current("t1.B"), 100, places=5
        )
    ...
    def test_trans_fw(self):
        ...
        self.assertTrue(
            0.8 <= res.get_current("t1.B") / (6 / 20e3) <= 1.2,
            f"wrong base current: {res.get_current('t1.B')}",
        )
        self.assertAlmostEqual(
            -res.get_current("t1.E") / beta_f, res.get_current("t1.B"), places=5
        )
        self.assertAlmostEqual(res.y_norm, 0)
    ...
    def test_trans_bw(self):
        ...
        self.assertAlmostEqual(
            res.get_current("t1.E") / beta_r, res.get_current("t1.B"), places=5
        )
        self.assertAlmostEqual(res.y_norm, 0)
...
class TestPwmVoltage(unittest.TestCase):
    def test1(self):
        ...
        v = 7
        duty = 0.3
        vs = PwmVoltage("A", v, f, duty)
        net.add_component("V", vs, ("V", "0"))
        r = 9
        net.addR("R", r, "V", "0")
        ana = Analysis(net)
        res = ana.transient(10 / f, 0.005 / f)
