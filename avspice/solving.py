...
def solve(
    ...
    while True:
        ...
        if verbose:
            ...
            print(
                f"iteration {iterations}, norm_y={norm_y}, norm_dx={np.linalg.norm(dx)}, norm_x ={norm_x} ----"
            )
        xn = x + dx
        yn = fn(xn)
        ...
        if close_enough(x, xn, abstol, reltol):
            return BasicSolution(
                x=xn, y=yn, dfx=dfx, iterations=iterations, norm_y=norm_y_n
            )
...
def solve_alfa(
    ...
    while True:
        ...
        if alfa < 1e-3:
            alfa = 0
        res = solve(
            solution_vec, f, df, abstol, reltol, maxiter, alfa=alfa, verbose=verbose
        )
        if isinstance(res, str):
            if verbose:
...
def bisect(f: Callable[[float], float], xl: float, xr: float) -> float:
